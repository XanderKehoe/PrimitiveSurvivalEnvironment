...
public:
	...
	static void Center(int x, int y); // Moves the camera so that the center of the screen is the given xy coords.
	static bool IsCoordOutsideCamView(int x, int y, int offset); // Returns true if given coords with an offset are inside the camera's view 
