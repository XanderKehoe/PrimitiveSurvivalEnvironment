...
bool Camera::IsCoordOutsideCamView(int x, int y, int offset) 
{
	/*printf("Centering check | %d < (%d * %d) + (%d * %d) |  %d < (%d * %d) + (%d * %d) | %d > (%d * %d) + (%d * %d) - %d | %d > (%d * %d) + (%d * %d) - %d\n",
		x, xPos, zoom, offset, zoom,
		y, yPos, zoom, offset, zoom,
		x, xPos, zoom, SCREEN_WIDTH, zoom, offset,
		y, yPos, zoom, SCREEN_HEIGHT, zoom, offset); */
	if (x < (xPos * zoom) + (offset * zoom) ||
		y < (yPos * zoom) + (offset * zoom) ||
		x > (xPos * zoom) + (SCREEN_WIDTH * zoom) - offset ||
		y > (yPos * zoom) + (SCREEN_HEIGHT * zoom) - offset)
	{
